...
export const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  try {
    ...
    } else if (req.url && req.url.match(URL_REG_EXP) && req.method === 'GET') {
    ...
    } else if (req.url && req.url.match(URL_REG_EXP) && req.method === 'PUT') {
      const id = req.url.split('/')[3];
    ...
    } else if (req.url && req.url.match(URL_REG_EXP) && req.method === 'DELETE') {
      const id = req.url.split('/')[3];
